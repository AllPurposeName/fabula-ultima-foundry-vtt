...
export class FabulaUltimaItem extends Item {
	...
	get state() {
		return this.getFlag("fabula-ultima", "state") || "";
	}
	...
	getRollModifier(...rollIdentifiers) {
		...
		const modifiers = Object.values(this.rollModifiers).reduce((array, mod) => {
			...
			const state = this.getFlag("fabula-ultima", "state");
	...
	async sendToChat() {
		...
		for (const field of ["Appearance", "Description", "Drawback", "Effect"]) {
			if (itemData.system[field.toLowerCase()] && !this.getFlag("fabula-ultima", field))
		...
		const html = await renderTemplate("systems/fabula-ultima/templates/components/item-chatcard.hbs", itemData);
		const chatData = {
			user: game.userId,
		...
		await message.setFlag("fabula-ultima", "itemData", itemData); // Adds posted item data to chat message flags for item drag/drop
	}
